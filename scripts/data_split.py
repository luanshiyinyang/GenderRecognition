...
np.random.seed(2020)
df_raw = pd.read_csv(os.path.join(cfg.ds_folder, 'Training.csv'), encoding="utf8")
...
def get_one_ds():
    ...
    pd.DataFrame({'id': train_images, 'label': train_labels}).to_csv(os.path.join(cfg.ds_folder, "new_train.csv"), index=False)
    pd.DataFrame({'id': valid_images, 'label': valid_labels}).to_csv(os.path.join(cfg.ds_folder, "new_valid.csv"), index=False)
